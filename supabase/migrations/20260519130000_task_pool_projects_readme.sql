-- README handbook (overview, install, issues, versions) on tasks and projects
ALTER TABLE public.task_pool_items ADD COLUMN IF NOT EXISTS readme text;
ALTER TABLE public.projects ADD COLUMN IF NOT EXISTS readme text;

COMMENT ON COLUMN public.task_pool_items.readme IS 'HTML handbook: overview, installation, known issues, versions, etc.';
COMMENT ON COLUMN public.projects.readme IS 'HTML handbook: overview, installation, known issues, versions, etc.';
...
BEGIN
  ...
  INSERT INTO public.projects (
    ...
    r.currency,
    r.github_url,
    COALESCE(r.published_links, '[]'::jsonb),
    now()
  )
