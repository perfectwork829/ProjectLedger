...
type Props = {
  /** MIME filter for the file input (e.g. all images, or any type). */
  accept: string;
  title: string;
  onUrlAdded: (url: string) => void;
};

export function CloudGoogleDriveUpload({ accept, title, onUrlAdded }: Props) {
  ...
  const connect = async () => {
    setLoading(true);
    try {
      sessionStorage.setItem('gdrv_oauth_return', `${window.location.pathname}${window.location.search}`);
      const state = crypto.randomUUID();
      sessionStorage.setItem('gdrv_oauth_state', state);
  ...
  const onFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const { data: tok, error: tErr } = await googleDriveAccessToken();
      if (tErr) throw tErr;
      const access = tok?.access_token;
      if (!access) throw new Error(tok?.error || 'Not connected to Google Drive');

      const url = await uploadFileToGoogleDrive(file, access);
      onUrlAdded(url);
  ...
  return (
    <div className="rounded-md border border-dashed bg-muted/20 p-3 space-y-2">
      ...
      <p className="text-[11px] text-muted-foreground">
        Files upload to your Google account (Drive). Ensure the Edge Function <code className="text-xs">cloud-storage-google</code> is deployed and{' '}
        <code className="text-xs">GOOGLE_CLIENT_ID</code> / <code className="text-xs">GOOGLE_CLIENT_SECRET</code> are set in Supabase.
      </p>
      <div>
        <input
          type="file"
