...
import { Calendar, MessageSquare, Pencil, Plus, Trash2, FolderKanban, Link2, Clock, ListTodo } from 'lucide-react';
...
export default function AdminTaskPool() {
  ...
  const stackTree = useMemo(() => {
    const grouped = filteredItems.reduce<Record<string, TaskPoolItemRecord[]>>((acc, p) => {
  ...
  const displayItems = useMemo(() => {
    if (selectedStack === 'all') return filteredItems;
    return filteredItems.filter((p) => (p.main_stack || 'uncategorized').toLowerCase() === selectedStack);
  }, [filteredItems, selectedStack]);
  ...
  const promoteIfNeeded = async (poolId: string, status: string) => {
    ...
    if (projectId) {
      toast({
        ...
        description: 'This completed task was copied into Projects.',
      });
      fetchAll();
    }
  };
  ...
  const updateSubtaskStatus = async (t: PoolSubtask, status: string) => {
    const res = await supabase.from('task_pool_subtasks').update({ status, updated_at: new Date().toISOString() }).eq('id', t.id);
    if (res.error) {
      toast({ title: 'Update failed', description: res.error.message, variant: 'destructive' });
      return;
    }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pre-project leads. Completing a task creates a matching Project automatically.
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <Card className="lg:col-span-1">
          ...
          <CardContent className="max-h-[70vh] overflow-auto">
            ...
            >
              All stacks ({filteredItems.length})
            </button>
              ...
              >
                ...
                {row.task_source ? (
                  <p className="mt-1 text-xs text-muted-foreground capitalize">
                    Task source(from): {row.task_source.replace('_', ' ')}
                  </p>
                ) : null}
                {row.main_stack ? (
        ...
        <Card className="lg:col-span-2">
          {!selected ? (
            <CardContent className="py-12 text-center text-muted-foreground">Select a task pool item.</CardContent>
          ) : (
            <>
                ...
                <Tabs defaultValue="overview" className="w-full">
                  ...
                  <TabsContent value="tasks" className="space-y-3">
                    <div className="grid gap-2 sm:grid-cols-[1fr_220px_auto]">
                      <Input value={newSubtaskTitle} onChange={(e) => setNewSubtaskTitle(e.target.value)} placeholder="Subtask title" />
                          ...
                          <div key={task.id} className="rounded border p-3 flex items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                onClick={() => setPendingDelete({ kind: 'subtask', id: task.id, title: task.title })}
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            </div>
